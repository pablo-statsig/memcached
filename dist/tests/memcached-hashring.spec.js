...
function getDistributionSpread(options) {
    const servers = [
        '203.0.113.117:12011',
        '203.0.113.114:12011',
        '203.0.113.121:12011',
    ...
    const memcached = new main_1.Memcached(servers, options);
    const hashRing = memcached._hashRing;
...
describe('Memcached hashring configuration', () => {
    it('uses a denser default hashring to reduce shard skew', () => {
        ...
        chai_1.assert.isBelow(defaultSpread, sparseSpread);
        chai_1.assert.isBelow(defaultSpread, 0.015);
    });
    it('allows overriding the hashring vnode count', () => {
        const servers = ['localhost:11211', 'localhost:11212'];
        const memcached = new main_1.Memcached(servers, { hashRingVnodeCount: 400 });
        const hashRing = memcached._hashRing;
        chai_1.assert.equal(hashRing.vnode, 400);
    });
});
//# sourceMappingURL=memcached-hashring.spec.js.map